...
- [attack-surface](attack-surface/SKILL.md) — maintain a living attack-surface inventory (`attacksurface.md`, kept in an un-tracked location) of every deployed system, vendor, site, and credential store: tech, hosting, auth, data, exposure per audience, defenses, platform-specific misconfigurations, and a testing cadence per system. Records inventory; delegates deep assessment to the bundled `assess-attack-surface` workflow (`attack-surface/assess-attack-surface.js`), which fans out per-dimension finders, adversarially verifies each finding, and recommends re-test frequency from criticality weighed against cost.
