...
Do not call the task done until: the Step 1 gate was applied (visible change confirmed, or nothing sent), the email speaks the recipient's vocabulary and claims only verified outcomes with partial ships named, it follows the three-part format, and any send happened only after explicit confirmation and returned a message id. Script checks done at build time (2026-06-29): `bash -n` passes, the recipient-list and body jq build the exact verified Resend body shape `{from,to[],subject,html,cc[]}`, and the script exits cleanly with a clear message when the API key is absent. Acceptance test: draft an update for the most recent thing the user shipped. Live paid send deferred to first real use.
